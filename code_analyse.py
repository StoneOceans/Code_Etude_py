# -*- coding: utf-8 -*-
"""Copie de Analyse d'une journée de vol.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Kn2Hj2Y8aZGQ0iUUbXHUgshs9v5OXSEF
...
def filtrer_vols(dataframe,filedate):
  ...
  return dataframe.loc[
      ((dataframe['deprealise'].str.startswith('LF')) |  # LF condition
...
output['textetrasaction3transaction'].value_counts()

mots_recherches = ["ALGR"]

texte = output[((output['deprealise'].str.startswith('LF')) |  # LF condition
...
df_stan_facture = df_stan_facture.sort_values(by='callsignprevu', ascending=True)

df_stan_facture = df_stan_facture.reset_index()

df_stan_facture.head()

df_stan_facture[df_stan_facture['callsignfinal']=='THY54SK']
...
def generate_transmission_ecmonth(df):
  ...
  df['transmission'] = ''
  df['EOBTprevu'] = df['EOBTprevu'].fillna(0).astype(int)
  df['adressemodeprevu'] = df['adressemodeprevu'].fillna(f"{' ' * 6}")
  df['IFPLprevu'] = df['IFPLprevu'].fillna(f"{' ' * 10}")
  ...
  for index, row in df.iterrows():
    ...
    transmission += row['depprevu']
    transmission += row['arrprevu']
    transmission += row['callsignprevu'].strip().ljust(9)
    transmission += 'Z'
    transmission += row['typeavionprevu'].strip().ljust(7)
    transmission += 'Z  '
    transmission += str(row['work1prevu']).strip().ljust(8)
    transmission +=  f"{' ' * 21}"
    transmission += str(row['date_blockprevu'])
    transmission += str(row['IFPLprevu']).strip().ljust(10) + f"{' ' * 39}" + str(int(row['EOBTprevu'])) + "    " + str(row['adressemodeprevu']).strip().ljust(6)
...
tableau['pln'].value_counts()

tableau["balise"].value_counts().get("VFR", 0)
